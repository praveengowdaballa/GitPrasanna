...
if date -v-180d > /dev/null 2>&1; then
      ...
      volume=$(aws ec2 describe-volumes --query 'Volumes[*].[VolumeId, Attachments[0].InstanceId,CreateTime]' --filters Name=status,Values=available --output table --
...
output text)
     ...
     echo "script for ebsvolume attached within 180 DAYS: $volume"
      ...
      echo "unsttached elastic ips are :$elasticips"
       ...
       StopedInstance=$(aws ec2 describe-instances --query 'Reservations[].Instances[].[BlockDeviceMappings[*].Ebs.VolumeId]' --filter "Name=instance-state-
...
query 'Reservations[].Instances[].SecurityGroups[].GroupId' --output text | tr '\t' '\n' | sort | uniq) --output table)
      ...
      ami=$(ws ec2 describe-images --owners=  --query "Status.available.Images[].State[]" --output table)
